...
export class DataObjectCrudComponent implements OnInit {
  ...
  public async getJson(): Promise<any> {
    try {
      ...
      const response = await firstValueFrom(this.httpClient.get("assets/dbObjectForm.json"));
      return response;
    } catch (error) {
      // Handle error appropriately
      console.error('Error fetching JSON:', error);
      throw error; // Re-throw the error or handle it in a way that makes sense for your application
    }
  }
  ...
  async ngOnInit() {
   ...
   // bsCustomFileInput.init()
    ...
    this.startDate.valueChanges.subscribe((newStartDate) => {
      // If endDate is less than startDate, update it to be equal to startDate
      if (this.endDate.value < newStartDate) {
        this.endDate.setValue(newStartDate);
      }
    });
    ...
    if (!(typeof this.data['form'] === 'undefined')) {
      console.log('  data form   ', JSON.stringify(this.data['form']))
      ...
      for (var i in this.formInput) {
        ...
        console.log(i, ' ', this.formInput[i]['label'], ' ', JSON.stringify(this.formInput[i]));
  ...
  onFileChanged(event: any) {
    ...
    console.log(' onFileChanged file name ', this.uploadFilePath?.name)
  ...
  async onSubmit() {
    ...
    this.formInputLoop.map(elem => {
      ...
      console.log(elem['label'], 'elem ', this.dataForm[elem['label'].replace(/ /g, '_')])
    ...
    console.log('action ', this.echangeObject.action, ' JSON.stringify(this.dataForm) ', JSON.stringify(this.dataForm))
    ...
    if (this.echangeObject.action == undefined || this.echangeObject.action == "details") {
      let data = this.buildOjectFromDataForm(this.dataForm)
